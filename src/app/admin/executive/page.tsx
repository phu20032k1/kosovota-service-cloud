      ...
      {loading && !data ? <LoadingState label="Đang tổng hợp dữ liệu doanh nghiệp..." /> : data && <>
          ...
          <article className="surface-card overflow-hidden">
            ...
            <div className="p-3 sm:p-5">
              <div className="grid grid-cols-3 gap-2 sm:grid-cols-6 sm:gap-4">
                {data.trend.map((row) => <div key={row.key} className="grid min-w-0 grid-rows-[145px_auto] gap-2 sm:grid-rows-[190px_auto] sm:gap-3">
                  <div className="flex items-end justify-center gap-1 rounded-2xl bg-slate-50 px-1.5 pt-3 sm:gap-1.5 sm:px-2 sm:pt-4">
        ...
        <section className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_minmax(0,1.4fr)]">
          ...
          <article className="surface-card">
            ...
            <div className="hidden overflow-x-auto md:block"><table className="min-w-full text-sm"><thead><tr><th className="p-3 text-left">Đại lý</th><th className="p-3 text-left">Rating</th><th className="p-3 text-left">Lệnh hoàn thành</th><th className="p-3 text-left">Doanh thu dịch vụ</th><th className="p-3 text-right">Chi tiết</th></tr></thead><tbody>{data.topDealers.map((dealer) => <tr key={dealer.id}><td className="p-3"><strong>{dealer.name}</strong><div className="text-xs text-slate-500">{dealer.dealerCode}</div></td><td className="p-3 font-black">{dealer.rating?.toFixed(1) || "—"}</td><td className="p-3 font-bold">{dealer.completed}</td><td className="p-3 font-black text-emerald-700">{money(dealer.revenue)}</td><td className="p-3 text-right"><Link href={`/admin/dealers/${dealer.id}`} className="btn-secondary whitespace-nowrap text-xs">Xem đại lý</Link></td></tr>)}{!data.topDealers.length && <tr><td colSpan={5} className="p-8 text-center text-slate-500">Chưa có dữ liệu đại lý.</td></tr>}</tbody></table></div>
            <div className="divide-y divide-slate-100 md:hidden">{data.topDealers.map((dealer)=><article key={dealer.id} className="p-4"><div className="flex items-start justify-between gap-3"><div className="min-w-0"><h3 className="break-words font-black text-slate-950">{dealer.name}</h3><p className="text-xs font-bold text-slate-500">{dealer.dealerCode}</p></div><span className="status-pill status-slate">★ {dealer.rating?.toFixed(1) || "—"}</span></div><div className="mt-3 grid grid-cols-2 gap-2"><div className="rounded-xl bg-slate-50 p-3"><span className="text-xs text-slate-500">Hoàn thành</span><p className="mt-1 text-lg font-black">{dealer.completed}</p></div><div className="rounded-xl bg-emerald-50 p-3"><span className="text-xs text-emerald-700">Doanh thu</span><p className="mt-1 break-words font-black text-emerald-700">{money(dealer.revenue)}</p></div></div><Link href={`/admin/dealers/${dealer.id}`} className="btn-secondary mt-3 w-full">Xem đại lý</Link></article>)}{!data.topDealers.length&&<p className="p-8 text-center text-slate-500">Chưa có dữ liệu đại lý.</p>}</div>
          </article>
        </section>
      </>}
    </div>
