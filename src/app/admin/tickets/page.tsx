...
const FINISHED_STATUSES = new Set(["RESOLVED", "CLOSED", "CANCELLED"]);
...
const STATUS_FILTER_LABEL: Record<string, string> = { ALL: "Tất cả", NEW: "Mới", ASSIGNED: "Đã giao", IN_PROGRESS: "Đang xử lý", WAITING_CUSTOMER: "Chờ khách hàng", RESOLVED: "Đã sửa", CLOSED: "Đã đóng", CANCELLED: "Đã hủy" };
...
export default function TicketsPage() {
  ...
  const stats = useMemo(() => ({
    open: data?.tickets.filter((ticket) => !FINISHED_STATUSES.has(ticket.status)).length || 0,
    critical: data?.tickets.filter((ticket) => ticket.priority === "CRITICAL" && !FINISHED_STATUSES.has(ticket.status)).length || 0,
  ...
  async function removeTicket(ticket: Ticket) {
    const linked = ticket.serviceOrder ? `\nLệnh Điều phối ${ticket.serviceOrder.orderCode} cũng sẽ bị xóa.` : "";
    if (!window.confirm(`Xóa yêu cầu ${ticket.ticketCode}?${linked}\n\nThao tác này không thể hoàn tác.`)) return;
    setBusy(true);
    try {
      ...
      if (!response.ok || !result.success) throw new Error(result.message || "Không xóa được yêu cầu");
      setSelectedId(null);
      showNotice("success", result.message || "Đã xóa yêu cầu.");
      await load();
    } catch (caught) {
      showNotice("error", caught instanceof Error ? caught.message : "Không xóa được yêu cầu");
    } finally {
      setBusy(false);
    }
  }
  ...
  return <main className="min-h-screen">
    ...
    <div className="page-container space-y-4">
      ...
      {loading && !data ? <LoadingState label="Đang tải yêu cầu hỗ trợ..."/> : data && <>
        ...
        <section className="surface-card flex flex-wrap items-center justify-between gap-3 px-4 py-3">
          <div><p className="text-xs font-black uppercase tracking-[.16em] text-emerald-700">Tổng quan nhanh</p><p className="mt-1 text-sm text-slate-600">Theo dõi phiếu hỗ trợ, lệnh điều phối và lịch sử trao đổi trong cùng một màn hình.</p></div>
        ...
        <section className="surface-card p-4">
          ...
          <div className="mt-3 flex gap-2 overflow-x-auto pb-1">
            {["ALL", "NEW", "ASSIGNED", "IN_PROGRESS", "WAITING_CUSTOMER", "RESOLVED", "CLOSED", "CANCELLED"].map((status) => <button type="button" key={status} onClick={() => setFilter(status)} className={`shrink-0 rounded-xl px-3 py-2 text-xs font-bold ${filter === status ? "bg-emerald-600 text-white" : "bg-slate-100 text-slate-600"}`}>{STATUS_FILTER_LABEL[status] || status}</button>)}
          </div>
        </section>
        ...
        <section className="ticket-workspace">
          <article className="surface-card overflow-hidden">
            ...
            <div className="ticket-list-scroll divide-y">
              {visible.map((ticket) => <button type="button" key={ticket.id} onClick={() => setSelectedId(ticket.id)} className={`ticket-list-item ${selected?.id === ticket.id ? "is-selected" : ""}`}>
                <div className="flex flex-wrap items-start justify-between gap-3"><div className="min-w-0"><p className="break-words font-black">{ticket.ticketCode} · {ticket.subject}</p><p className="mt-1 text-sm text-slate-600">{ticket.contactName} · {ticket.contactPhone}</p><div className="mt-2 flex flex-wrap gap-2 text-xs text-slate-500">{ticket.machine && <span className="mini-chip"><Icon name="droplet" size={13}/>{ticket.machine.id}</span>}{ticket.serviceOrder && <span className="mini-chip mini-chip--green"><Icon name="map" size={13}/>{ticket.serviceOrder.orderCode}</span>}{ticket.status === "RESOLVED" && <span className="mini-chip mini-chip--green">Đã sửa xong</span>}{!ticket.serviceOrder && ticket.machine && <span className="mini-chip mini-chip--amber">Chưa tạo lệnh</span>}</div><p className="mt-2 text-xs text-slate-400">Tạo {date(ticket.createdAt)} · Hạn {date(ticket.dueAt)}</p></div><div className="flex flex-wrap gap-2"><StatusBadge value={ticket.priority}/><StatusBadge value={ticket.status}/></div></div>
              </button>)}
          ...
          <aside className="surface-card ticket-detail-panel">
            {selected ? <TicketDetail ticket={selected} data={data} reply={reply} setReply={setReply} busy={busy} update={update} removeTicket={removeTicket}/> : <div className="empty-state"><div><Icon name="chevron-right" size={30} className="mx-auto mb-2"/><p className="font-bold">Chọn một yêu cầu để xử lý</p></div></div>}
          </aside>
        </section>
      </>}
    </div>
    ...
    {open && <div className="modal-backdrop" onMouseDown={(event) => { if (event.target === event.currentTarget) setOpen(false); }}>
      <form onSubmit={create} className="modal-panel max-w-4xl">
        ...
        </div><RequirementRestatement form={form} machineLabel={data?.machines.find((machine) => machine.id === form.machineId)?.name || data?.machines.find((machine) => machine.id === form.machineId)?.model}/></div>
...
function TicketDetail({ ticket, data, reply, setReply, busy, update, removeTicket }: { ticket: Ticket; data: Data; reply: string; setReply: (value: string) => void; busy: boolean; update: (payload: Record<string, unknown>) => Promise<void>; removeTicket: (ticket: Ticket) => Promise<void> }) {
  const finished = FINISHED_STATUSES.has(ticket.status);
  return <div>
    <div className="border-b p-5">
      ...
      <button type="button" disabled={busy} onClick={() => void removeTicket(ticket)} className="mt-4 rounded-xl border border-rose-200 bg-rose-50 px-4 py-2 text-sm font-black text-rose-700 disabled:opacity-50">Xóa yêu cầu{ticket.serviceOrder ? " & lệnh Điều phối" : ""}</button>
    </div>
    <div className="space-y-5 p-5">
      {ticket.status === "RESOLVED" && <Notice kind="success"><strong>Đã sửa xong.</strong> Báo cáo hoàn thành đã được hệ thống ghi nhận. Các chức năng xử lý của yêu cầu này đã được khóa.</Notice>}
      {ticket.status === "CLOSED" && <Notice kind="info">Yêu cầu đã đóng. Các chức năng xử lý đã được khóa.</Notice>}
      {ticket.status === "CANCELLED" && <Notice kind="warning">Yêu cầu đã hủy. Các chức năng xử lý đã được khóa.</Notice>}

      <section className="requirement-summary"><div className="flex items-center gap-2"><span className="summary-icon"><Icon name="file" size={18}/></span><div><h3 className="font-black">Mô tả lại yêu cầu</h3><p className="text-xs text-slate-500">Thông tin đã được chuẩn hóa để CSKH, Điều phối và Đại lý cùng hiểu.</p></div></div><dl className="summary-grid mt-4"><Summary label="Khách hàng" value={`${ticket.contactName} · ${ticket.contactPhone}`}/><Summary label="Máy cần xử lý" value={ticket.machine ? `${ticket.machine.id} · ${ticket.machine.name || ticket.machine.model}` : "Chưa xác định"}/><Summary label="Loại / ưu tiên" value={`${TYPE_LABEL[ticket.type] || ticket.type} · ${ticket.priority === "CRITICAL" ? "Khẩn cấp" : ticket.priority === "HIGH" ? "Cao" : ticket.priority === "LOW" ? "Thấp" : "Bình thường"}`}/><Summary label="Mong muốn" value={ticket.description || ticket.subject}/></dl></section>
      ...
      <section className={`dispatch-link-card ${ticket.serviceOrder ? "is-linked" : ""}`}>
        ...
        {ticket.serviceOrder ? <div className="mt-4 rounded-2xl bg-white/80 p-4"><div className="flex flex-wrap items-center justify-between gap-3"><div><p className="text-xs font-bold text-emerald-700">{ticket.serviceOrder.orderCode}</p><p className="mt-1 font-black">{ticket.serviceOrder.serviceType}</p><p className="mt-1 text-xs text-slate-500">{ticket.serviceOrder.dealer?.name || "Chưa giao đại lý"} · Hạn {date(ticket.serviceOrder.dueDate)}</p></div><div className="flex flex-wrap gap-2"><Link href={`/admin/service-orders/${ticket.serviceOrder.id}`} className="btn-secondary text-xs"><Icon name="eye" size={15}/>Xem chi tiết lệnh</Link><Link href={`/operations-map?order=${ticket.serviceOrder.id}`} className="btn-primary px-3 py-2 text-xs font-black text-white"><Icon name="map" size={15}/>Mở Điều phối</Link></div></div></div> : ticket.machine && !finished ? <div className="mt-4"><Notice kind="warning">Phiếu cũ chưa có lệnh liên kết. Bấm nút dưới để tạo ngay, không cần nhập lại dữ liệu.</Notice><button type="button" disabled={busy} onClick={() => update({ ensureServiceOrder: true })} className="btn-primary mt-3 w-full justify-center px-4 py-3 font-black text-white disabled:opacity-60"><Icon name="plus" size={17}/>Tạo lệnh Điều phối từ phiếu</button></div> : !ticket.machine ? <div className="mt-4"><Notice kind="warning">Chưa xác định máy nên chưa thể chuyển sang Điều phối.</Notice></div> : null}
      </section>

      {!finished && <><div className="grid gap-3 sm:grid-cols-2">
...
function RequirementRestatement({ form, machineLabel }: { form: TicketForm; machineLabel?: string }) {
  ...
  return <section className="requirement-summary"><div className="flex items-center gap-2"><span className="summary-icon"><Icon name="file" size={18}/></span><div><h3 className="font-black">Mô tả lại yêu cầu</h3><p className="text-xs text-slate-500">Kiểm tra nội dung hệ thống sẽ chuyển cho bộ phận Điều phối.</p></div></div><dl className="summary-grid mt-4"><Summary label="Người yêu cầu" value={`${form.contactName || "Chưa nhập"} · ${form.contactPhone || "Chưa nhập"}`}/><Summary label="Thiết bị" value={form.machineId ? `${form.machineId}${machineLabel ? ` · ${machineLabel}` : ""}` : "Chưa chọn máy"}/><Summary label="Nhu cầu" value={`${TYPE_LABEL[form.type] || form.type}: ${form.subject || "Chưa nhập tiêu đề"}`}/><Summary label="Luồng sau khi lưu" value={willDispatch ? "Tự động tạo lệnh mới/đã giao và hiển thị ngay tại Điều phối" : "Chỉ lưu phiếu; cần bổ sung máy để tạo lệnh Điều phối"}/></dl></section>;
}
