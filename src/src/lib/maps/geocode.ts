...
export async function geocodeAddress(address: string): Promise<GeocodeResult | null> {
  if (process.env.GEOCODING_ENABLED !== "true") return null;
  const value = address.trim();
  if (!value) return null;
  const provider = (process.env.GEOCODING_PROVIDER || (process.env.GOOGLE_MAPS_API_KEY ? "google" : process.env.NEXT_PUBLIC_MAP_PROVIDER) || "maptiler").toLowerCase();
  if (provider === "google") {
    const key = process.env.GOOGLE_MAPS_API_KEY || process.env.GOOGLE_MAPS_SERVER_API_KEY || process.env.NEXT_PUBLIC_GOOGLE_MAPS_API_KEY;
    if (!key) throw new Error("Thiếu GOOGLE_MAPS_API_KEY.");
