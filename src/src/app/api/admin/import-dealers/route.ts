  ...
  try {
    ...
    for (const { data: row, rowNumber } of parsedRows) {
      try {
        ...
        let lat = numberOrNull(value(row, "Vĩ độ", "Latitude", "lat"));
        let lng = numberOrNull(value(row, "Kinh độ", "Longitude", "lng"));
        const fullAddress = [address, province].filter(Boolean).join(", ");
        if ((lat === null || lng === null) && fullAddress && process.env.GEOCODING_ENABLED === "true") {
          try {
            const location = await geocodeAddress(fullAddress);
            if (location) { lat = location.lat; lng = location.lng; }
          } catch (geocodeError) {
            console.warn(`Không geocode được đại lý dòng ${rowNumber}:`, geocodeError);
          }
        }
