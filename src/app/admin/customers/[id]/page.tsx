...
type CustomerResponse = { success: boolean; message?: string; data?: Customer; gpsBackfilledCount?: number; gpsWarning?: string };
...
function warrantyEndDate(base?: string | null, months?: number | null) {
  if (!base || !months || months <= 0) return null;
...
export default function CustomerDetailPage() {
  ...
  const [gpsNotice, setGpsNotice] = useState<{ kind: "success" | "info"; text: string } | null>(null);
  const [activity, setActivity] = useState({ type: "CALL", summary: "", detail: "", nextContactAt: "" });
  const load = useCallback(async () => {
    setLoading(true); setError("");
    try {
      const response = await fetch(`/api/crm/customers/${id}`, { cache: "no-store" });
      const result = await response.json() as CustomerResponse;
      if (!response.ok || !result.success || !result.data) throw new Error(result.message || "Không tải được hồ sơ");
      setData(result.data);
      if ((result.gpsBackfilledCount || 0) > 0) {
        setGpsNotice({ kind: "success", text: `Đã tự ghim GPS cho ${result.gpsBackfilledCount} máy từ địa chỉ khách hàng. Máy đã sẵn sàng để tìm đại lý trong tab Điều phối.` });
      } else if (result.gpsWarning) {
        setGpsNotice({ kind: "info", text: result.gpsWarning });
      } else {
        setGpsNotice(null);
      }
    }
  ...
  return <main className="min-h-screen"><OperationsHeader title={data?.name || "Hồ sơ khách hàng"} subtitle={data ? `${data.phone} · ${data.machines.length} thiết bị` : "CRM khách hàng 360°"} actions={<Link href="/admin/customers" className="btn-secondary"><Icon name="users" size={16}/>Danh sách</Link>}/><div className="page-container space-y-6">
    {message && <Notice kind="success">{message}</Notice>}{error && <Notice kind="error">{error}</Notice>}{gpsNotice && <Notice kind={gpsNotice.kind}>{gpsNotice.text}</Notice>}
    {loading && !data ? <LoadingState label="Đang mở hồ sơ khách hàng..."/> : data && <>
      <section className="grid gap-4 xl:grid-cols-[1fr_2fr]"><article className="surface-card p-5"><div className="flex items-start justify-between"><div><p className="text-xs font-extrabold uppercase tracking-widest text-emerald-700">Hồ sơ khách hàng</p><h2 className="mt-1 text-2xl font-black">{data.name}</h2></div><span className={`status-badge ${data.segment === "VIP" ? "badge-violet" : "badge-slate"}`}>{data.segment || "STANDARD"}</span></div><div className="mt-5 space-y-3 text-sm"><p><strong>Điện thoại:</strong> <a className="text-emerald-700" href={`tel:${data.phone}`}>{data.phone}</a></p><p><strong>Địa chỉ:</strong> {data.address || "Chưa cập nhật"}</p><p><strong>CSKH phụ trách:</strong> {data.owner?.name || "Chưa phân công"}</p><p><strong>Liên hệ gần nhất:</strong> {date(data.lastContactAt)}</p><p><strong>Lịch tiếp theo:</strong> {date(data.nextContactAt)}</p></div><div className="mt-5 grid grid-cols-2 gap-3"><label><span className="mb-1 block text-xs font-bold">Phân khúc</span><select value={data.segment || "STANDARD"} onChange={(event) => void update({ segment: event.target.value })}><option>STANDARD</option><option>VIP</option><option>AT_RISK</option><option>INTERNAL</option></select></label><label><span className="mb-1 block text-xs font-bold">Hài lòng</span><select value={data.satisfaction || ""} onChange={(event) => void update({ satisfaction: Number(event.target.value) })}><option value="">Chưa đánh giá</option>{[1, 2, 3, 4, 5].map((number) => <option key={number}>{number}</option>)}</select></label></div></article>
        <article className="surface-card"><div className="data-toolbar"><div><h2 className="page-section-title">Thiết bị đang sở hữu</h2><p className="page-section-subtitle">Mở hồ sơ khách hàng sẽ tự ghim GPS cho máy chưa có tọa độ từ địa chỉ khách hàng; GPS hợp lệ sẵn có không bị ghi đè.</p></div></div><div className="grid gap-4 p-5 md:grid-cols-2">{data.machines.map((machine) => { const completed = machine.serviceOrders.filter((order) => ["COMPLETED", "CLOSED"].includes(order.status)).length; const activation = machine.activations.at(-1)?.createdAt; const warrantyBase = activation || machine.installDate; const warrantyEnd = warrantyEndDate(warrantyBase, machine.warrantyMonths); const gpsPinned = hasUsableCoordinates(machine.lat, machine.lng); return <div key={machine.id} className="soft-card p-4"><div className="flex justify-between gap-3"><div><Link href={`/admin/machines/${encodeURIComponent(machine.id)}`} className="font-black text-emerald-700">{machine.name || machine.model}</Link><p className="text-xs text-slate-500">Seri: {machine.serial || machine.id} · Model: {machine.model}</p></div><div className="flex flex-col items-end gap-1"><StatusBadge value={machine.status}/><span className={`status-badge ${gpsPinned ? "badge-blue" : "badge-slate"}`}>{gpsPinned ? "GPS đã ghim" : "GPS chưa ghim"}</span></div></div><div className="mt-3 grid gap-1 text-xs text-slate-600 sm:grid-cols-2"><p><strong>Thời gian SX:</strong> {date(machine.manufactureDate, false)}</p><p><strong>Ngày lắp đặt:</strong> {date(machine.installDate, false)}</p><p><strong>Thời hạn BH:</strong> {machine.warrantyMonths ? `${machine.warrantyMonths} tháng` : "—"}</p><p><strong>Kích hoạt BH:</strong> {date(activation, false)}</p><p className="sm:col-span-2"><strong>Hết hạn BH:</strong> {date(warrantyEnd, false)}</p><p className="sm:col-span-2"><strong>Vị trí máy:</strong> {gpsPinned ? `${machine.lat!.toFixed(6)}, ${machine.lng!.toFixed(6)}` : "Chưa có tọa độ hợp lệ"}</p></div><div className="mt-4 grid grid-cols-3 gap-2 text-center"><div className="rounded-xl bg-white p-2"><strong>{machine.serviceOrders.length}</strong><p className="text-[10px] text-slate-500">Tổng lệnh</p></div><div className="rounded-xl bg-white p-2"><strong>{completed}</strong><p className="text-[10px] text-slate-500">Đã xử lý</p></div><div className="rounded-xl bg-white p-2"><strong>{machine.serviceOrders.length - completed}</strong><p className="text-[10px] text-slate-500">Chưa xử lý</p></div></div><div className="mt-3 flex flex-wrap gap-2"><Link href={`/admin/machines/${encodeURIComponent(machine.id)}`} className="btn-secondary px-3 py-2 text-xs"><Icon name="droplet" size={15}/>Hồ sơ máy</Link><Link href={`/operations-map?machine=${encodeURIComponent(machine.id)}`} className="btn-secondary px-3 py-2 text-xs"><Icon name="map" size={15}/>Mở Điều phối</Link></div></div>; })}{!data.machines.length && <p className="text-sm text-slate-500">Khách hàng chưa có thiết bị.</p>}</div></article></section>
