...
export async function GET(request: NextRequest) {
  ...
  const [machines, customers, dealers, ordersMonth, ordersPrev, overdueSchedules, upcomingSchedules, openTickets, criticalTickets, inventoryValueRows, lowStockRows, paymentBatches, provinceRows, topDealers] = await Promise.all([
    prisma.machine.count(), prisma.customer.count(), prisma.dealer.count({ where: { status: "APPROVED" } }),
  ...
  return NextResponse.json({ success: true, data: {
    kpis: { machines, customers, dealers, revenueMonth, revenuePrev, overdueSchedules, upcomingSchedules, openTickets, criticalTickets, inventoryValue, paid, payable },
