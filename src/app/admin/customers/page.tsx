...
export default function CustomersPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (!response.ok || !result.success || !result.data) throw new Error(result.message || "Không tải được CRM");
  ...
  const allSelected = data.length > 0 && data.every((customer) => selectedIds.includes(customer.id));

  function toggleAll() {
    if (allSelected) setSelectedIds([]);
    else setSelectedIds(data.map((customer) => customer.id));
  }
  ...
  function openEdit(customer: Customer) {
    ...
    setForm({
      ...
      satisfaction: customer.satisfaction ? String(customer.satisfaction) : "",
    });
  }
  ...
  return <main className="min-h-screen">
    <OperationsHeader title="Khách hàng 360°" subtitle="Tìm kiếm, sửa, xóa và quản lý hồ sơ khách hàng" />
      ...
      <section className="surface-card overflow-hidden">
        <div className="data-toolbar">
          <div className="min-w-0 flex-1">
            ...
            <p className="mt-2 text-xs font-bold text-slate-500">Đang hiển thị {data.length} khách hàng</p>
          </div>
        ...
        <div className="action-bar m-4">
          ...
          <div className="flex flex-wrap items-center gap-2">
            <span className="status-pill status-slate">Đã chọn: {selectedIds.length}</span>
            <button type="button" disabled={busy || selectedIds.length === 0} onClick={() => setDeleteIds(selectedIds)} className="danger-button text-sm disabled:opacity-50">
              <Icon name="trash" size={16} /> Xóa đã chọn
            </button>
            <button type="button" disabled={busy || data.length === 0} onClick={() => setDeleteIds(data.map((customer) => customer.id))} className="ghost-danger text-sm disabled:opacity-50">
              Xóa toàn bộ đang lọc
            </button>
          </div>
        </div>

        {loading ? <LoadingState label="Đang tìm khách hàng..." /> : <div className="admin-data-scroll">
          <table className="min-w-[1180px] w-full text-sm">
            <thead>
              <tr>{["Chọn", "Khách hàng", "Phân khúc", "Thiết bị", "Tương tác", "Ticket", "CSKH", "Liên hệ tiếp", "Đánh giá", "Thao tác"].map((header) => <th key={header} className="p-3 text-left">{header}</th>)}</tr>
            </thead>
            <tbody>
              {data.map((customer) => <tr key={customer.id} className="border-b align-top hover:bg-slate-50">
                <td className="p-3"><input type="checkbox" checked={selectedIds.includes(customer.id)} onChange={() => toggleOne(customer.id)} /></td>
                ...
                <td className="sticky right-0 bg-white p-3 shadow-[-8px_0_12px_-12px_rgba(15,23,42,.4)]">
                  <div className="flex gap-2 whitespace-nowrap">
                    <Link href={`/admin/customers/${customer.id}`} className="btn-secondary px-3 py-2 text-xs"><Icon name="eye" size={15} /> Xem</Link>
                    <button type="button" onClick={() => openEdit(customer)} className="btn-secondary px-3 py-2 text-xs"><Icon name="edit" size={15} /> Sửa</button>
                    <button type="button" onClick={() => setDeleteIds([customer.id])} className="ghost-danger px-3 py-2 text-xs"><Icon name="trash" size={15} /> Xóa</button>
                  </div>
                </td>
              </tr>)}
              {!data.length && <tr><td colSpan={10} className="p-10 text-center text-slate-500">Không có khách hàng phù hợp.</td></tr>}
            </tbody>
          </table>
        </div>}
      </section>
    ...
    {editing && <div className="modal-backdrop" onMouseDown={(event) => { if (event.target === event.currentTarget) setEditing(null); }}>
      <div className="modal-panel max-w-2xl">
        <div className="modal-header"><div><p className="eyebrow">Khách hàng</p><h2 className="mt-1 text-2xl font-black">Sửa thông tin</h2></div><button type="button" onClick={() => setEditing(null)} className="icon-button"><Icon name="x" size={18} /></button></div>
        <div className="modal-body grid gap-4 sm:grid-cols-2">
          <label><span className="mb-2 block text-sm font-bold">Tên khách hàng</span><input value={form.name} onChange={(event) => setForm({ ...form, name: event.target.value })} /></label>
          <label><span className="mb-2 block text-sm font-bold">Số điện thoại</span><input value={form.phone} onChange={(event) => setForm({ ...form, phone: event.target.value.replace(/\D/g, "") })} /></label>
          <label className="sm:col-span-2"><span className="mb-2 block text-sm font-bold">Địa chỉ</span><input value={form.address} onChange={(event) => setForm({ ...form, address: event.target.value })} /></label>
          <label><span className="mb-2 block text-sm font-bold">Phân khúc</span><select value={form.segment} onChange={(event) => setForm({ ...form, segment: event.target.value })}><option value="STANDARD">Tiêu chuẩn</option><option value="VIP">VIP</option><option value="AT_RISK">Có nguy cơ</option><option value="INTERNAL">Nội bộ</option></select></label>
          <label><span className="mb-2 block text-sm font-bold">Đánh giá</span><select value={form.satisfaction} onChange={(event) => setForm({ ...form, satisfaction: event.target.value })}><option value="">Chưa đánh giá</option>{[1,2,3,4,5].map((value) => <option key={value} value={value}>{value}/5</option>)}</select></label>
        </div>
        <div className="modal-footer"><button type="button" onClick={() => setEditing(null)} className="btn-secondary">Hủy</button><button type="button" disabled={busy} onClick={() => void saveEdit()} className="btn-primary px-5 py-3 font-black text-white disabled:opacity-50">Lưu thay đổi</button></div>
      </div>
    </div>}
    ...
    <ConfirmDialog
      ...
      tone="danger"
      title={deleteIds?.length === 1 ? "Xóa khách hàng?" : "Xóa nhiều khách hàng?"}
      description="Hồ sơ CRM và lịch sử tương tác sẽ bị xóa. Máy và Ticket được giữ lại nhưng sẽ không còn liên kết với khách hàng này."
      highlight={deleteIds?.length === 1 ? data.find((customer) => customer.id === deleteIds[0])?.name || "1 khách hàng" : `${deleteIds?.length || 0} khách hàng`}
